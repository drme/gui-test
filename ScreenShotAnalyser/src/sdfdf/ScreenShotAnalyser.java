...
public class ScreenShotAnalyser {
	...
	public static void main(String[] args) throws IOException {
		...
		String inputDir = System.getProperty("user.dir") + "\\samples\\";
		String outputDir = System.getProperty("user.dir") + "\\out\\";
		...
		for (final File fileEntry : new File(inputDir).listFiles()) {
			if (false == fileEntry.isDirectory()) {
				parseImage(fileEntry, outputDir);
			}
		}
	};

	private static void parseImage(File file, String outDir) throws IOException {
		...
		Imgcodecs.imwrite(new File(outDir, file.getName()).getAbsolutePath(), original);
	};
	...
	private static void extractTexts(File file, Mat original, Mat mask, List<MatOfPoint> contours) throws IOException {
		...
		for (MatOfPoint contour : contours) {
			...
			// double fillRatio = (double) Core.countNonZero(maskROI) / (bounds.width *
			// bounds.height);
			...
			// else
			{
				// Imgproc.rectangle(original, bounds.br(), bounds.tl(), new Scalar(255, 255,
				// 0), 2);
			}
		}
	};
